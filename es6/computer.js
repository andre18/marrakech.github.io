class Computer {
    ...
    naming() {
        document.getElementById(this.gamerNameHTMLid).innerHTML = this.name;
    }
    ...
    putCarpet() {
        ...
        for (let i = 0; i < bestCarpetVariants.length; i++) {
            if (bestCarpetVariants[i].weight === max) {
    ...
    checkLeftCell () {
        ...
        if (tmpX >= 0) {
            ...
            for (let i = 0; i < carpetVariants.length; i++) {
                if (carpetVariants[i].weight === max) {
                    // maxArr.push(max);
                    maxArr.push(carpetVariants[i].weight);
                }
                if (carpetVariants[i].weight > max) {
                    max = carpetVariants[i].weight;
                    maxArr = [];
                    maxArr.push(max);
                }
            }
